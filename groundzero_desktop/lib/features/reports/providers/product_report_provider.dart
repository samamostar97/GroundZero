...
final productReportProvider =
    NotifierProvider<ProductReportNotifier, ProductReportState>(
        ProductReportNotifier.new);

class ProductReportNotifier extends Notifier<ProductReportState> {
  late final ReportsRepository _repository;

  @override
  ProductReportState build() {
    _repository = ref.watch(reportsRepositoryProvider);
    final now = DateTime.now();
    final from = DateTime(now.year - 1, now.month, now.day);
    Future.microtask(() => loadData());
    return ProductReportState(isLoading: true, from: from, to: now);
  }
  ...
  Future<void> loadData() async {
    ...
    try {
      final data = await _repository.getProductData(
        from: state.from,
        to: state.to,
      );
  ...
  Future<void> exportReport(String format, BuildContext context) async {
    ...
    final path = await FilePicker.platform.saveFile(
      ...
      fileName: 'izvjestaj-proizvodi.$ext',
    ...
    try {
      final bytes = await _repository.downloadReport(
        endpoint: ApiConstants.productReport,
        from: state.from,
        to: state.to,
